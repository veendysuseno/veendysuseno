<!-- Header -->

<h1 align="center">
  <img src="https://raw.githubusercontent.com/MartinHeinz/MartinHeinz/master/wave.gif" width="28px">
  Hi, I'm <strong>Veendy Suseno</strong>
</h1>

<p align="center">
  <strong>IT Specialist | Network Engineer | Network Security Enthusiast</strong>
  <br>
  IT Infrastructure • Networking • System Administration • Cyber Security
  <br><br>
  <em>Building, troubleshooting, securing, and continuously learning technology.</em> 🚀
</p>

---

## 👨‍💻 About Me

I'm an **IT Specialist and Network Engineer** with a strong interest in **IT Infrastructure, Computer Networking, Network Security, and Cyber Security**.

My technical interests cover infrastructure and networking from the fundamentals to security-oriented environments, including **Windows, Linux, Active Directory, Windows Server, Cisco, MikroTik, firewalls, VPN, and network troubleshooting**.

I also enjoy exploring **programming, web development, data analytics, IoT, and automation** to expand my understanding of technology beyond infrastructure.

* 📍 Jakarta, Indonesia
* 🎓 Computer Engineering — Universitas Gunadarma
* 💼 IT Specialist / Network Engineer
* 🌐 Focused on **Networking & IT Infrastructure**
* 🔐 Strong interest in **Network Security & Cyber Security**
* 🖥️ Interested in **Windows Server, Active Directory & Linux**
* 🌐 Networking: **Cisco & MikroTik**
* 📊 Exploring **Data Analytics & Automation**
* 🤖 Interested in **IoT & emerging technologies**
* 🧑‍💻 Learning **Web Development & Programming**
* 🚀 Currently building projects, labs, and technical knowledge

> **Learn → Build → Troubleshoot → Secure → Improve**

---

## 🎯 Professional Focus

My current technical direction is centered around:

```text
IT Support
    │
    ├── Hardware & Software Troubleshooting
    ├── Windows / Linux
    └── System Administration
          │
          ▼
Network Engineering
    │
    ├── Routing & Switching
    ├── Cisco
    ├── MikroTik
    ├── LAN / WLAN
    └── Network Troubleshooting
          │
          ▼
Network Security
    │
    ├── Firewall
    ├── VPN
    ├── Network Hardening
    └── Security Monitoring
          │
          ▼
Cyber Security
    │
    ├── Security Fundamentals
    ├── Vulnerability Assessment
    ├── Defensive Security
    ├── Network Security
    └── Security Operations
```

---

## 🛠️ Technical Skills

### 🖥️ IT Infrastructure & System Administration

![Windows](https://img.shields.io/badge/Windows-0078D4?style=flat\&logo=windows\&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat\&logo=linux\&logoColor=black)
![Windows Server](https://img.shields.io/badge/Windows%20Server-0078D4?style=flat\&logo=windows\&logoColor=white)
![Active Directory](https://img.shields.io/badge/Active%20Directory-0078D4?style=flat\&logo=microsoft\&logoColor=white)

* Windows 10 / 11
* Linux
* Windows Server
* Active Directory
* User & Access Management
* Hardware & Software Troubleshooting
* System Administration
* IT Support & Helpdesk
* Printer & Peripheral Troubleshooting

---

### 🌐 Networking

![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?style=flat\&logo=cisco\&logoColor=white)
![MikroTik](https://img.shields.io/badge/MikroTik-293239?style=flat\&logo=mikrotik\&logoColor=white)

* TCP/IP & OSI Model
* Routing & Switching
* LAN / WLAN
* IP Addressing & Subnetting
* DHCP / DNS
* VLAN
* Network Troubleshooting
* MikroTik Configuration
* Cisco Networking
* Network Monitoring
* Network Infrastructure

---

### 🔐 Network & Cyber Security

![Fortinet](https://img.shields.io/badge/Fortinet-EE3124?style=flat\&logo=fortinet\&logoColor=white)
![Kali Linux](https://img.shields.io/badge/Kali%20Linux-557C94?style=flat\&logo=kalilinux\&logoColor=white)

* Network Security
* Firewall
* VPN
* Security Fundamentals
* Vulnerability Assessment
* Security Hardening
* Cyber Security Fundamentals
* Security Monitoring
* Penetration Testing Fundamentals
* Linux Security Labs
* Network Security Labs

---

### 💻 Programming & Development

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat\&logo=html5\&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat\&logo=css3\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat\&logo=javascript\&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat\&logo=python\&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat\&logo=cplusplus\&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat\&logo=github\&logoColor=white)

* HTML5
* CSS3
* JavaScript
* Python
* C++
* Git
* GitHub
* Web Development
* Automation & Scripting

---

### 📊 Data & Emerging Technology

![Python](https://img.shields.io/badge/Python-3776AB?style=flat\&logo=python\&logoColor=white)

* Data Analytics
* Data Processing
* Python-based Automation
* IoT
* Technology Research
* Experimentation & Technical Labs

---

## 🏅 Certifications

* 🏆 **Fortinet NSE 5**
* 🏆 **Fortinet NSE 4 — Network Security**
* 🏆 **MTCNA — MikroTik Certified Network Associate**
* 🏆 **BNSP — Computer Technical Support**
* 🏆 **Google Cybersecurity Professional Certificate — Coursera**
* 🏆 **Google IT Support Professional Certificate — Coursera**

---

## 🚀 Featured Projects

### 📋 TaskFlow

A lightweight **Task Manager web application** built without a frontend framework.

**Technology:**

* HTML5
* CSS3
* Vanilla JavaScript
* LocalStorage

**Features:**

* Task Management
* Task Filtering
* Search
* Priority System
* Deadline
* Progress Tracking
* Task Statistics
* Dark Mode
* Responsive Design
* LocalStorage Persistence

---

## 🧪 Technical Labs

I use personal labs and virtual environments to continuously improve my practical skills.

### 🌐 Networking Labs

* MikroTik RouterOS
* Routing & Switching
* VLAN
* DHCP
* DNS
* Firewall
* VPN
* Network Troubleshooting
* Network Monitoring

### 🔐 Cyber Security Labs

* Kali Linux
* Nmap
* Burp Suite
* Web Security Fundamentals
* Vulnerability Identification
* Network Security
* Defensive Security
* Security Monitoring

### 🐧 Linux Labs

* Linux Administration
* Bash
* File Permissions
* Users & Groups
* Processes
* Networking
* Package Management
* Virtual Environments

---

## 📚 Currently Learning

```text
Networking
├── Advanced Routing & Switching
├── Network Security
├── Firewall
└── Network Troubleshooting

Cyber Security
├── Security Fundamentals
├── Network Security
├── Vulnerability Assessment
├── Defensive Security
└── SOC / Security Monitoring

System Administration
├── Linux
├── Windows Server
├── Active Directory
└── Infrastructure Management

Programming
├── JavaScript
├── Python
├── Web Development
└── Automation

Emerging Technology
├── Data Analytics
└── IoT
```

---

## 📊 GitHub Profile

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=veendysuseno&label=Profile%20Views&color=0e75b6&style=for-the-badge" alt="veendysuseno" />
  <img src="https://img.shields.io/github/followers/veendysuseno?style=for-the-badge" alt="GitHub Followers" />
  <img src="https://img.shields.io/github/stars/veendysuseno?style=for-the-badge" alt="GitHub Stars" />
</p>

---

## 🌐 Connect With Me

<p align="center">

<a href="https://www.linkedin.com/in/veendysuseno/" target="_blank">
<img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="LinkedIn" height="30" width="40"/>
</a>

<a href="https://instagram.com/veendysuseno" target="_blank">
<img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" alt="Instagram" height="30" width="40"/>
</a>

<a href="https://veendysuseno.medium.com/" target="_blank">
<img src="https://github.com/veendysuseno/github-profile-readme-generator/blob/main/src/images/icons/Social/medium.svg" alt="Medium" height="30" width="40"/>
</a>

<a href="mailto:veendysuseno@gmail.com">
<img src="https://github.com/tandpfun/skill-icons/blob/main/icons/Gmail-Dark.svg" alt="Email" height="30" width="40"/>
</a>

</p>

<p align="center">

---

## 🐍 Contribution Snake

<div align="center">
  <img src="https://profile-readme-generator.com/assets/snake.svg" alt="Snake animation" />
</div>

---

## 💡 Philosophy

<p align="center">
  <strong>Technology is not just about knowing how things work.</strong>
  <br>
  It's about understanding, building, troubleshooting, securing, and continuously improving them.
</p>

---

<p align="center">
  ⭐ <em>Feel free to explore my repositories and projects.</em>
  <br><br>
  🔥 <strong>Let's build, learn, and secure technology together.</strong>
</p>

---

<p align="center">
  <sub>© 2026 Veendy Suseno</sub>
</p>
